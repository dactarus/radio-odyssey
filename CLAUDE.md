...
*Dernière mise à jour de ce fichier : 2026-08-16 (§81).*
...
- **108 pages éditoriales** — organisées en 7 catégories définies dans `src/data/navigation.js` : Bien-être & Santé (18), Musique & Énergie (26), Playlists du Jour (6), Artistes & Styles (10), Conseils d'Écoute (8), Les Coulisses (12), International (13 pages en `lang="en"`, servies sous `/en/<slug>.html` depuis le §81). S'y ajoutent l'accueil, `/plan-du-site.html`, `/quiz-musicaux-radio-odyssey.html` et les deux pages légales.
...
- **Internationalisation** (§81, chantier en cours) : les 13 pages anglaises sont regroupées sous `/en/<slug>.html`, avec 13 redirections 301 depuis leurs anciennes URLs. Le `hreflang` de `Layout.astro`, jusque-là auto-référent avec un `x-default` codé en dur vers l'accueil français, est remplacé par une prop `altLangs={{ fr: '…', en: '…' }}` — ⚠️ **à ne renseigner qu'entre deux pages traitant réellement du même sujet** ; cinq paires vérifiées à ce jour, les huit autres pages anglaises sont des originaux sans équivalent français et n'émettent aucun `hreflang`, ce qui est correct. ⚠️ Avec `build.format: 'file'`, l'URL `/en/` n'existe pas : l'accueil anglais sortira à **`/en.html`**. **Restent à faire** : accueil anglais, traduction de trois pages de fond, E-E-A-T sur les deux pages anglaises de cohérence cardiaque, gabarit de fiche artiste anglais, un ou deux quiz. ⚠️ **La navigation reste en français sur les 13 pages anglaises.**
...
11. **Décision internationale** — tranchée au §81 : ni traduction intégrale des 255 pages, ni repli, mais une **sélection assumée de portes d'entrée anglophones** pour un public froid. Lot 1 fait (les 13 pages sous `/en/`, 301, `hreflang` corrigé) ; lots 2 à 5 détaillés en fin de §81
