...
*Dernière mise à jour de ce fichier : 2026-09-25 (§101).*

## Objectif du projet

Faire de **radio-odyssey.com** un site de contenu riche — objectif des 100 pages thématiques **atteint et dépassé** (296 pages générées, 278 au sitemap) — organisé en catégories :
...
**278 URL au sitemap** (296 pages générées, dont 18 fiches artistes volontairement désindexées — §61, §63), réparties en deux silos :

- **Pages éditoriales** — organisées en 7 catégories définies dans `src/data/navigation.js` : Bien-être & Santé (18), Musique & Énergie (31, dont les quiz « Cette année-là » et « Artiste ou groupe original » depuis les §98-99), Playlists du Jour (6), Artistes & Styles (10), Conseils d'Écoute (8), Les Coulisses (13), International (13 pages en `lang="en"`, servies sous `/en/<slug>.html` depuis le §81). S'y ajoutent l'accueil, `/plan-du-site.html`, `/quiz-musicaux-radio-odyssey.html` et les deux pages légales. ⚠️ Le total exact de ce silo n'a pas été recompté dans son ensemble depuis un moment — seuls les chiffres ci-dessus ont été vérifiés directement dans `navigation.js` (dernière fois au §100) ; se fier au compte réel (`sitemap.xml`, `dist/`) plutôt qu'à un total additionné à la main avant de trancher une décision qui en dépend.
...
| `QuizBlock.astro` | Moteur de quiz réutilisable (§69) — les 5 quiz par décennie gardent leur copie propre, les 3 quiz thématiques français et le quiz des records anglais l'utilisent. **Bilingue depuis le §94** : prop `lang`, libellés dans `QUIZ_UI` (`data/i18n.js`). ⚠️ Les 4 chaînes produites par le script passent par des attributs `data-`, jamais par `define:vars` |
...
| `i18n.js` (dans `src/data/`) | **Source unique des libellés d'interface** (§82) — `UI.fr`/`UI.en`, helper `t(lang)`, `QUIZ_UI`/`quizT(lang)` pour le moteur de quiz (§94), et `EN_NAV_CATEGORIES`, la navigation anglaise. ⚠️ Toute chaîne visible ajoutée à un composant de mise en page doit passer par là, sinon elle réapparaît en français sur les 18 pages anglaises |
...
| `NewsletterCapture.astro` | Capture e-mail minimale (§74) — formulaire natif posté vers Brevo (double opt-in), sur les 10 pages de quiz et les 8 pages avec exercice de cohérence cardiaque |

**Données** (`src/data/`) : `artists-en.js` (§86 — **contenu anglais des fiches artistes** ; ⚠️ la présence d'un slug y décide qu'une fiche anglaise existe, et les `facts` sont des traductions de la version française, jamais des textes nouveaux), `navigation.js` (menu, hubs, pied de page), `artists.js` (147 fiches — **source unique** des passages), `genres.js`, `discoveries.js`, `icons.js`, `reviewer.js` (attribution et sources des pages bien-être, §72).
...
- ⚠️ **Le service worker ne s'enregistre que sur `www.radio-odyssey.com`** (§87). Ailleurs — `localhost`, aperçu Astro, préversions Vercel — il ne s'installe pas : il s'intercalait entre le navigateur et le serveur et a provoqué **trois diagnostics faux** (§84, §86, §87). Conséquence : le mode hors ligne et l'installation ne se testent qu'en production.
...
**À savoir** : `app.radio-odyssey.com` est une **seconde PWA**, sur un dépôt distinct (dossier `Appli Radio Odyssey V8`). Sauvegardée depuis le 2026-08-17 (§88) sur `github.com/dactarus/radio-odyssey-app`, **dépôt privé** — il contient `admin.html` et `api/send.js`, dont le mot de passe d'administration est comparé en clair et sans limitation des tentatives : ne pas le rendre public. ⚠️ **La publication reste `npx vercel --prod`**, elle ne passe pas par git : le dépôt GitHub n'est **pas** rattaché au projet Vercel `radio-odyssey-v8b`, et ne doit pas l'être sans décision explicite — sinon chaque `git push` déclencherait un déploiement, y compris d'un commit de travail. ⚠️ Aucun secret n'est en dur dans ce dépôt, ni dans son historique (vérifié au §88) : tout passe par les variables d'environnement Vercel (`VAPID_PRIVATE_KEY`, `UPSTASH_REDIS_REST_TOKEN`, `PUSH_ADMIN_SECRET`). ⚠️ `npx vercel --prod` publie le **dossier entier**, pas seulement ce que suit git — d'où les règles `*.avant-*`, `*.backup`, `*.bak` ajoutées à `.vercelignore`.
...
- **Groupe Radio Odyssey (§101)** : l'entrée de menu vers `relax.radio-odyssey.com` est pilotée par `src/data/groupe.js`. ⚠️ Passer `RELAX_LANCEE` à `true` le 17 novembre 2026 (lancement public de Relax). Jumeau côté dépôt `radio-odyssey-relax`, même chemin, rôles inversés.

- ⚠️ **Poids du HTML** : une explication va dans le **frontmatter** d'un composant, ou en `{/* … */}` dans le gabarit — **jamais dans une zone que le build recopie telle quelle**. Un commentaire placé dans un `<script>` en ligne est téléchargé par toutes les pages (2 339 o récupérés au §85) ; un commentaire `<!-- … -->` aussi (**1 048 Ko récupérés au §95**, la même erreur sous une autre forme). Seul `{/* … */}` est retiré à la construction. Même vigilance sur les icônes SVG rendues en ligne : `globe` pèse 1 098 octets pièce.
...
- **Silo artistes** : traité aux §61, §62 et §63, décliné en anglais au §86 (20 fiches). ⚠️⚠️ **DÉCISION ARRÊTÉE, NE PAS LA REFAIRE : les fiches françaises gardent le rang, les fiches anglaises ne l'affichent pas.** Ce n'est pas une incohérence, c'est une différence d'intention de recherche, et elle a été tranchée explicitement par le propriétaire le 2026-08-17 :
  - **En français**, le rang et la part d'antenne *reflètent la radio*. Le lecteur francophone connaît la station, ou vient la découvrir : la fiche fait les deux à la fois, informer sur l'artiste et rendre compte de la programmation. Le rang y est une information sur nous, et il est lu comme tel. **Il n'y a rien à y changer.**
  - **En anglais**, la recherche porte sur la **notoriété de l'artiste**, pas sur la station — qui est inconnue du visiteur. On ratisse volontairement large avec de très gros noms (ABBA, The Beatles, Queen), et sur ces noms-là un « 137ᵉ sur 147 » ne se lit pas « notre grille tourne peu cet artiste » mais « cet artiste est mineur ». Le chiffre est exact, la lecture qu'il induit est fausse, et elle dessert précisément les pages censées servir de porte d'entrée. Les deux tuiles de rang y sont donc remplacées par le nombre de titres en rotation et la fréquence hebdomadaire — tout reste calculé, rien n'est inventé (§62 tient).
  ⚠️ Corollaire pour la page de regroupement anglaise : elle classe **par époque**, jamais par volume de diffusion (§87).
- **Internationalisation** (§81-§84, chantier en cours) : **18 pages anglaises** — accueil `/en.html` + 17 pages `/en/<slug>.html` —, avec 13 redirections 301 depuis les anciennes URLs. ⚠️ Avec `build.format: 'file'`, l'URL `/en/` n'existe pas : l'accueil anglais est `/en.html` (deux 301 rattrapent `/en/` et `/en/index.html`). **Interface** : `src/data/i18n.js` porte tous les libellés ; `Header`, `MegaNav`, `Footer`, `Sidebar`, `SearchModal`, `CookieConsent`, `WebviewBanner` et `RadioPlayer` prennent une prop `lang` que `Layout.astro` leur transmet. ⚠️ `#topRadioBar` et le bandeau navigateur portent `transition:persist` : leurs libellés restent figés dans la langue de la première page chargée. **Navigation anglaise** : `EN_NAV_CATEGORIES` (Listen / Calm & Focus / About), structure propre et non traduite du menu français, sans page de hub. **Maillage** : ⚠️ sur une page anglaise, un lien interne n'est affiché que si sa cible existe en anglais — deux exceptions seulement, les mentions légales/confidentialité (obligation légale) et le renvoi vers l'accueil français depuis la barre latérale de `/en.html`. **`hreflang`** : prop `altLangs={{ fr: '…', en: '…' }}` sur `Layout.astro`, remplaçant un `hreflang` auto-référent et un `x-default` codé en dur vers l'accueil français ; ⚠️ à ne renseigner qu'entre deux pages traitant réellement du même sujet — 10 paires vérifiées, les 8 autres pages anglaises sont des originaux sans équivalent et n'émettent aucun `hreflang`, ce qui est correct. ⚠️ **Pagefind indexe par langue** (`fr` 245 pages, `en` 18) : une recherche depuis une page anglaise ne remonte que de l'anglais. **E-E-A-T** : posé sur les deux pages anglaises de cohérence cardiaque au §83. **Sélecteur de langue** (§84) : dans la barre de menu, le panneau mobile et le pied de page ; il vise la page équivalente si le couple `altLangs` existe, sinon l'accueil de l'autre langue. ⚠️ Ses liens portent `data-astro-reload` — sans chargement complet, la barre du lecteur (`transition:persist`) garderait les libellés de la langue précédente. **Fiches artistes** : 20 fiches anglaises (§86), regroupées sur `/en/artists-on-radio-odyssey.html` et accessibles par le groupe « Artists » du menu anglais (§87). ⚠️ Ni les fiches ni la page de regroupement n'affichent de rang ou de tri par diffusion. **Quiz** : **huit quiz anglais** — les cinq par décennie (§89-§90) et les trois thématiques : records (§94), noms de scène (§96), coulisses (§97) —, regroupés sur `/en/music-quizzes.html` en deux groupes (*by decade* / *by theme*) et accessibles par le groupe « Quizzes » du menu anglais. ⚠️ Les questions des cinq quiz par décennie sont reprises **à l'identique** du français — titres et noms d'artistes ne se traduisent pas — donc une correction d'un côté doit être répercutée de l'autre. ⚠️ Les **quiz thématiques**, eux, ne sont **pas des décalques** : un énoncé en prose se réévalue au lieu de se traduire. Records : 19 questions communes sur 20 (§94). Noms de scène : 16 sur 20, et une reformulée — le verlan de « maestro » n'existe pas en anglais (§96). Coulisses : 17 sur 20 (§97). ⚠️ **Règle absolue : un fait de remplacement doit déjà être publié et vérifié dans `artists.js`.** On ne crée jamais un fait à l'occasion d'une traduction (§86). ⚠️ Trois contrôles à passer par script sur toute nouvelle page de quiz, qu'aucun build ne fait : bonnes réponses toutes distinctes, tous les slugs connus d'`artists.js`, et chaque bonne réponse présente dans ses propres propositions. ⚠️⚠️ **Le critère de remplacement est la notoriété de l'œuvre chez le lecteur visé, PAS la nationalité de l'artiste.** Corrigé par le propriétaire au §97 : *« Jain est connu à l'international, pas les 3 autres »* — la question sur « Makeba » a été conservée alors que je l'avais classée à retirer. Un artiste français peut être parfaitement lisible à l'étranger ; c'est la chanson qui décide. **Les trois quiz thématiques sont désormais traduits.** Le bloc « in our quizzes » des fiches anglaises est alimenté par `data/quiz-artistes-en.js`, qui ne lit que `src/pages/en/*quiz*.astro` — ⚠️ **le nom du fichier doit contenir « quiz »**, c'est le glob qui décide. **43 pages anglaises**, du §81 au §97.
...
1. **Mesurer l'effet des trois nouveaux quiz** (§69) avant d'en créer d'autres : le réservoir permettrait un quatrième quiz, mais rien ne dit qu'un neuvième quiz apporte plus qu'il ne dilue. Les cinq quiz par décennie ont valu au site sa première citation dans un Aperçu IA (§59) — regarder si les nouveaux font de même.
   ⚠️ La question affichée est « Qui chante « … » ? » / « Who sings “…”? » (§90) — mais le champ `track` reste le **titre nu** : `data/quiz-artistes.js` l'extrait pour les fiches artistes, y mettre la question entière casserait le croisement. ⚠️ Le croisement quiz ↔ fiches est automatisé par `data/quiz-artistes.js`, qui lit le code source des pages au build. Format attendu : `{ track: "…", correct: 'slug' }` ou `{ enonce: "…", correct: 'slug' }`. **Si l'extraction ne trouve rien, le bloc quiz disparaît des 147 fiches sans erreur au build** — c'est arrivé au §69.
...
2. ~~**Corriger l'étiquetage de « L'assasymphonie » dans le manager RadioKing**~~ — corrigé par le propriétaire (2026-08-25) : le titre était crédité à Emmanuel Moire, c'est bien celui de Florent Mothe (*Mozart, l'opéra rock*).
   **Tom York** a désormais sa fiche « Découverte » (`src/data/discoveries.js`, hors silo des 147 fiches — pas de `playCount`/`tracks` RadioKing car ces deux champs se dérivent uniquement du rapport de diffusion, jamais à la main). Complétée le 2026-08-25 avec des faits transmis par mail : label **D.A Red Wolf Records**, remixeur pour OneRepublic et Lara Fabian, clips diffusés sur C Star, Clubbing TV et **Bblack Africa**, trois singles sortis en 2025 : *Close To Me* (mars), *Sunscreen* (juillet), *I Can't Stay* (feat. Noize) — dates confirmées par le propriétaire le 2026-08-25 après une contradiction dans les mails reçus.
