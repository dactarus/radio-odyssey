...
## 99. Dixième quiz : « Artiste ou groupe original », et une chasse aux reprises menée à quatre mains (2026-09-03)

Le propriétaire a proposé un dixième quiz d'un genre encore différent : Radio Odyssey diffuse parfois une **reprise** sans le préciser — l'artiste qu'on entend n'est pas celui qui a créé le titre. Question posée : « Quel artiste ou groupe original avait chanté ce titre ? » L'artiste diffusé est donné dans l'énoncé (ce n'est jamais la bonne réponse), 4 propositions, comme les huit premiers quiz.

**Méthode** : passage en revue des 522 titres réels du catalogue (`artists.js`, tous artistes confondus, pas seulement ceux déjà croisés dans un quiz), repérage des candidats plausibles à une recherche par connaissance générale, puis **vérification individuelle par recherche web de chaque candidat avant toute inclusion** — même rigueur que pour la série par décennie. Plusieurs pistes plausibles écartées faute de preuve : "I Feel For You" de Bob Sinclar n'a aucun rapport avec le standard de Prince/Chaka Khan malgré le titre identique ; "Take Me To Your Heart" de Rick Astley et "Put A Little Love On Me" de Delegation sont bien des originaux, pas des reprises ; "Coco Câline" de Julien Doré, un temps suspectée d'être une reprise de Lio, est bien un original (2017).

**Un quiz construit à quatre mains.** Le propriétaire a fourni cinq des douze pistes en cours de rédaction, chacune vérifiée avant intégration comme les autres : "Comme Caroline" de Zaho reprend "Caroline" de MC Solaar (1991) ; "Toutes les femmes de ta vie" de Julien Doré — qu'on aurait pu croire originale — reprend en réalité le tube 2001 du groupe L5, extrait de son album de reprises "Imposteur" (2024) ; "San Francisco" de Sound Of Legend (2026) reprend le classique hippie de Scott McKenzie (1967) ; "Paradise" de DJ Snake (2025) reprend "Another Day In Paradise" de Phil Collins (1989) ; "Forever Young" de David Guetta reprend le tube 1984 du groupe allemand Alphaville.

**Cas particulier de "Forever Young"** : ce titre n'apparaît pas dans les `tracks` de la fiche David Guetta — la fenêtre de l'export RadioKing qui alimente ce champ n'est pas à jour dessus. Le propriétaire, en tant que programmateur réel de l'antenne, a confirmé sa diffusion effective. Décision prise : utiliser le fait dans le quiz (contenu édité, comme les fiches Découvertes), mais **ne pas l'ajouter à la main dans `tracks`** — ce champ ne se saisit qu'au rapport de diffusion, jamais manuellement (règle rappelée en tête d'`artists.js`), pour ne pas casser la garantie qu'il reste ré-extractible à l'identique à chaque rafraîchissement.

**Conséquence assumée sur la taille du quiz** : le réservoir de reprises identifiables avec certitude dans le catalogue est nettement plus mince que pour les autres formats. Plutôt que de forcer 20 questions avec des pistes fragiles (leçon du §48 : mieux vaut un quiz plus court que des faits approximatifs), ce premier lot s'arrête à **12 questions vérifiées, sans round bonus**. Il grandira si de nouvelles reprises rejoignent la programmation. Un artiste diffusé n'apparaît qu'une fois, à l'exception assumée de Jimmy Somerville (deux reprises distinctes — Harold Melvin & The Blue Notes et Sylvester —, aucun conflit possible puisque les bonnes réponses sont différentes).

**Maillage** : chaque question renvoie vers la fiche de l'artiste réellement diffusé (« → Découvrir X sur Radio Odyssey »), pas vers les artistes originaux — la plupart d'entre eux (Shocking Blue, les Valentine Brothers, Harold Melvin & The Blue Notes...) ne sont pas diffusés sur Radio Odyssey et n'ont donc pas de fiche. ⚠️ **Ce quiz n'est volontairement pas branché sur le croisement automatique `data/quiz-artistes.js`** : son champ `correct` porte le nom de l'artiste *original* (la bonne réponse), pas un slug de fiche RO — l'inverse de la convention des neuf autres quiz, où `correct` désigne toujours l'artiste diffusé. Brancher ce quiz sur le système existant afficherait donc, à tort, l'artiste diffusé comme "bonne réponse" sur sa propre fiche. Le fichier utilise `titre` au lieu de `track`/`enonce` précisément pour ne pas matcher la regex d'extraction. Piste pour un lot futur : étendre `quiz-artistes.js` pour reconnaître ce second type de relation (« artiste diffusé, sujet de la question ») plutôt que de l'ignorer.

**Page de série mise à jour** : la troisième famille (ouverte au §55) devient plurielle — `QUIZ_ANNEE` renommé `QUIZ_TRANSVERSAUX`, nouvelle section "Les quiz transversaux", fiche `meta` par carte plutôt qu'un texte "10 questions + round bonus" commun (qui aurait été faux pour ce quiz à 12 questions sans bonus). `QuizInvite.astro` (accueil) a reçu une prop `totalQuestions` explicite : l'ancien calcul `boutonNombre * 20` supposait silencieusement que chaque quiz compte 20 questions, ce qui n'est plus vrai depuis ce lot — corrigé avant que l'erreur ne s'affiche en production (192 questions réelles contre 200 calculées).

**Vérifié après build (295 pages, +1, aucune erreur)** : script de contrôle confirmant les 12 bonnes réponses (les originaux) toutes distinctes, la bonne réponse présente dans ses 4 propositions à chaque question, les 11 slugs d'artistes diffusés tous vérifiés contre `artists.js` ; parcours complet testé dans le navigateur (build statique servi localement) — score final 12/12 confirmé, lien vers chaque fiche artiste fonctionnel.
